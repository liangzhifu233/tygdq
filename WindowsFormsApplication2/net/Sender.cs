...
using System.Text.Json; // 确保包含这个命名空间
...
namespace Net
{
    public class Sender
    {
        ...
        public static string PostObject(string url, Dictionary<string, string> headers,
            Dictionary<string, object> content)
        {
            ...
            Task<HttpResponseMessage> task = Sender.client.PostAsync(url, httpContent);
            try
            {
                return Sender.RunTask(task);
            }
